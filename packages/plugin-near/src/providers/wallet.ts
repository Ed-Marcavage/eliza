...
/**
 * Connect to the NEAR network using the provided runtime and retrieve the account information.
 * @param {IAgentRuntime} runtime - The runtime used for connecting to the NEAR network.
 * @returns {Promise<Account>} The NEAR account associated with the connection.
...
/**
 * Fetch the current value of the portfolio by getting the account balance, converting it to NEAR and USD,
 * fetching the NEAR price in USD, and calculating the total value in USD.
 * If the portfolio value is already cached, it will return the cached value.
 * @param {IAgentRuntime} runtime - The AgentRuntime interface for NEAR wallet integration.
 * @returns {Promise<WalletPortfolio>} - A promise that resolves to the WalletPortfolio object containing the total balance in USD and NEAR,
 * as well as token details for NEAR Protocol.
 * @throws {Error} - It throws an error if there is any issue fetching the portfolio or converting values.
 */
...
/**
 * Asynchronously fetches the near price from the Coingecko API.
 * If the price is found in the cache, it returns the cached price.
 * If the price is not found in the cache, it makes a request to the API to fetch the price.
 * If the request is successful, it stores the price in the cache and returns the fetched price.
 * If an error occurs during the process, it logs the error and returns 0.
 * 
 * @returns {Promise<number>} The near price fetched from the API or the cached price, or 0 in case of error.
 */
...
/**
 * Formats the portfolio information into a string with account ID, total value, token balances, and market prices.
 *
 * @param {IAgentRuntime} runtime - The agent runtime context.
 * @param {WalletPortfolio} portfolio - The portfolio object containing account information.
...
/**
 * Asynchronously retrieves the portfolio value using the provided IAgentRuntime instance and returns it in a formatted string.
 * 
 * @param {IAgentRuntime} runtime - The IAgentRuntime instance to use for fetching the portfolio value.
 * @returns {Promise<string>} A Promise that resolves to the formatted portfolio value as a string.
 */
