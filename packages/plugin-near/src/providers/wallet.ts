...
/**
 * Interface for representing a near token.
...
/**
 * Constructor for creating an instance of AccountManager.
...
/**
 * Asynchronously retrieves a formatted portfolio using the given agent runtime.
 * 
 * @param {IAgentRuntime} runtime - The agent runtime to use for retrieving the portfolio.
 * @param {Memory} _message - The memory object, not used in this method.
 * @param {State} [_state] - The optional state object.
 * @returns {Promise<string | null>} A promise that resolves to a formatted portfolio string or null if an error occurs.
 */
    async get(
 ...
 * 
 * @param {IAgentRuntime} runtime - The runtime object provided.
...
/**
 * Fetches data from the provided URL with retry mechanism.
 * @async
 ...
 * If the value is present in the cache, it returns the cached value.
 * Otherwise, it connects to the account, retrieves the balance, converts NEAR balance to NEAR token, 
 * fetches the NEAR price in USD, calculates the total USD value, and constructs a WalletPortfolio object.
 * The constructed portfolio is then cached using the cache key.
 * 
...
/**
 * Fetches the current NEAR price in USD from the Coingecko API.
 * If the price is already cached, returns it directly.
 * If not, makes a request to the API and caches the retrieved price.
 * 
 * @returns {Promise<number>} A promise that resolves to the NEAR price in USD.
 */
 ...
 * 
 * @param {IAgentRuntime} runtime - The agent runtime containing character system information.
...
/**
 * Asynchronously obtains and formats the user's portfolio information.
 * 
