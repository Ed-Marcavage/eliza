 ...
 * @property {string} [valueNear] - The value of the token in NEAR, if available.
 */
...
/**
 * Interface representing a wallet portfolio with total USD value, total NEAR value (optional), and an array of NearToken objects.
...
/**
 * Establishes a connection to NEAR wallet using the provided agent runtime.
 * If the account is already connected, the existing account is returned.
 * Otherwise, retrieves NEAR wallet credentials from the runtime settings,
 * validates them, creates a KeyPair from the secret key, sets the key
 * in the keystore, connects to the NEAR node using the provided configuration,
 * and returns the connected account.
 * @param {IAgentRuntime} runtime - The agent runtime to use for connecting to NEAR wallet.
...
/**
 * Fetches data from a URL with optional request options and retries if there are errors.
 * @param {string} url - The URL to fetch data from.
 * @param {RequestInit} [options={}] - The optional request options to include.
...
/**
 * Fetches the current value of the portfolio by connecting to the account using the provided runtime.
 * If the portfolio value is already cached, it returns the cached value.
 * Otherwise, it fetches the account balance, converts the balance to NEAR and fetches the NEAR price in USD to calculate the total USD value.
 * Then, it creates a WalletPortfolio object with the total USD value, total NEAR balance, and token details and stores it in the cache.
 * 
 * @param {IAgentRuntime} runtime - The runtime used to connect to the account.
 * @returns {Promise<WalletPortfolio>} The WalletPortfolio object containing total USD value, total NEAR balance, and token details.
 */
...
/**
 * Fetches the NEAR price from an API and caches the result for future use.
 * 
...
/**
 * Format the portfolio information into a string format.
 * 
 * @param {IAgentRuntime} runtime - The agent runtime object.
 * @param {WalletPortfolio} portfolio - The wallet portfolio object.
...
/**
 * Asynchronously retrieves the portfolio value using the provided IAgentRuntime object and returns it formatted as a string.
 * 
 * @param {IAgentRuntime} runtime - The IAgentRuntime object used to fetch portfolio information.
 * @returns {Promise<string>} A Promise that resolves with the formatted portfolio value or an error message if unable to fetch the information.
 */
